...
def nssa_ceiling(currency: str) -> Decimal:
    ...
    if normalise(currency) in {"usd", "us dollar", "us dollars"}:
        return ceiling_usd
    # The ceiling is maintained in USD by default. A local-currency ceiling can
    # be supplied by the implementation through custom_zimbabwe_nssa_ceiling.
    return ceiling_usd
...
def derive_slip_amounts(slip: Any) -> SlipAmounts:
    ...
    if basic and (not employee_nssa or not employer_nssa):
        calculated_employee_nssa, calculated_employer_nssa = nssa_pobs(basic, nssa_ceiling(currency))
